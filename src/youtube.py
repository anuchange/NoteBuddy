...
class YouTubeHandler:
    def __init__(self):
        ...
        self.mins = 15
        self.MAX_CHUNK_DURATION = self.mins * 60 * 1000  # 2 minutes in milliseconds
        # Configure rate limiting
        self.RATE_LIMIT_DELAY = 2  # Delay between chunks in seconds
        self.MAX_RETRIES = 3        # Maximum retry attempts for failed chunks
    ...
    def split_audio_into_chunks(self, audio_path):
        ...
        try:
            ...
            st.info(f"Total audio duration: {total_duration/1000/60:.2f} minutes")
            st.info(f"Processing in {num_chunks} chunks ({self.mins} minutes each)")
            ...
            for i in range(num_chunks):
                ...
                st.info(f"Chunk {i+1} size: {file_size/1024/1024:.2f} MB")
            ...
            try:
                ...
                if 'rate_limit_exceeded' in error_message:
                    if retry < self.MAX_RETRIES - 1:
                        ...
                        continue
                st.error(f"Error processing chunk {chunk_index + 1}: {error_message}")
                return None
    ...
    def get_transcript(self, video_id):
        ...
        try:
            ...
            st.info("Attempting to fetch official transcript...")
        ...
        try:
            ...
            for i, chunk_path in enumerate(chunk_paths):
                with st.spinner(f"Transcribing part {i + 1} of {total_chunks}..."):
                    if i > 0:
                        st.info(f"Waiting {self.RATE_LIMIT_DELAY} seconds...")
